...
static ncclResult_t initTransportsRank(struct ncclComm* comm, ncclUniqueId* commId) {
  ...
  for (int c=0; c<comm->nChannels; c++) {
    ...
    snprintf(line+strlen(line), 1023-strlen(line), " [%d] %d/%d/%d->%d->%d comm %p nRanks %02d busId %lx",
        c, tree->down[0], tree->down[1], tree->down[2], rank, tree->up, comm, comm->nRanks, comm->busId);
